...
dotOn (int displayIndex)
{
  HAL_GPIO_WritePin (g_display[displayIndex].segment[DOTINDEX].port,
		     g_display[displayIndex].segment[DOTINDEX].pin, GPIO_PIN_RESET);
}
...
{
  HAL_GPIO_WritePin (g_display[displayIndex].segment[DOTINDEX].port,
		     g_display[displayIndex].segment[DOTINDEX].pin, GPIO_PIN_SET);
}
...
void
displayDigit (const int displayIndex, const int number)
{
...
void
displayFloat (const int displayIndex, const float number)
{
  if(number > 99)
    {
